#--------固定不變--------#
dirPath = r"\\10.231.199.10\Department\InformationTechnology\Auditing\12. IR\NEW"   #照片存放路徑 
...
#    sheet.clear()
    ...
    cell_list = sheet.range('D2:D'+str(len(data)+1))   #A2:An  n= 取得字串長度+1
    ...
    with open(Output1, 'r') as infile:
        while True:
            ...
            if not data:                 # 所有資料讀取完畢
                break
            #print(data[:6]+"\n", end='')          # end='': 不要自動加斷行
            IR_only.append(data[:Use_bit]) #使用自訂參數
        #print("暫存-移除副檔名:"+str(IR_only))
...
def outputNew():
    ...
    with open(Output2, 'w') as outfile:
        while j < len(IR_only):
            outfile.write(str(IR_only[j])+',')
            j = j + 1
    #print("匯出-新資料:"+str(IR_only))    
