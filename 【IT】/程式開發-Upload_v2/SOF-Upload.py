...
#    sheet.clear()
    ...
    cell_list = sheet.range('E2:E'+str(len(data)+1))   #A2:An  n= 取得字串長度+1
    ...
    with open(Output1, 'r') as infile:
        while True:
            ...
            if not data:                 # 所有資料讀取完畢
                break
            #print(data[:6]+"\n", end='')          # end='': 不要自動加斷行
            SOF_only.append(data[:Use_bit]) #使用自訂參數
        #print("暫存-移除副檔名:"+str(SOF_only))

#---------轉出SOF號碼另存output.txt--------#    
def outputNew():
    j=0
    with open(Output2, 'w') as outfile:
        while j < len(SOF_only):
            outfile.write(str(SOF_only[j])+',')
            j = j + 1
    #print("匯出-新資料:"+str(SOF_only))    
