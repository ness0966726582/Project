#--------固定不變--------#
dirPath = r"\\10.231.250.70\Department\InformationTechnology\Auditing\10. JO\NEW"   #照片存放路徑 
Use_bit=40                    #取檔案名位數目的為移除副檔名
...
def Sendsheet():
    ...
    ###################################
    data=JO_only
    x=2   #列暫存初始B(googlesheet)
    y=2   #行暫存初始2(googlesheet)
    z=0   #陣列的初始位置

#    sheet.clear()
    sheet.update_cell(1,x, "Check JO List")
    '''  
    while z < len(data):
    ...
    cell_list = sheet.range('B2:B'+str(len(data)+1))   #A2:An  n= 取得字串長度+1
    ...
    with open(Output1, 'r') as infile:
        while True:
            ...
            if not data:                 # 所有資料讀取完畢
                break
            #print(data[:6]+"\n", end='')          # end='': 不要自動加斷行
            JO_only.append(data[:Use_bit]) #使用自訂參數
        #print("暫存-移除副檔名:"+str(JO_only))
...
def outputNew():
    ...
    with open(Output2, 'w') as outfile:
        while j < len(JO_only):
            outfile.write(str(JO_only[j])+',')
            j = j + 1
    #print("匯出-新資料:"+str(JO_only))    
